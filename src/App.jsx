...
// Icon mark — compact serif "O" initial derived from new brand wordmark
const OkombaLogo = ({ size = 38, className = "" }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" fill="none"
    xmlns="http://www.w3.org/2000/svg" className={className}
    role="img" aria-label="OKOMBA Analytics Logo mark">
    {/* Background — dark ground matching brand pill */}
    <rect width="100" height="100" rx="22" fill="#0B0F1A"/>
    {/* Subtle gold border for definition against dark page backgrounds */}
    <rect width="100" height="100" rx="22" fill="none" stroke="rgba(240,165,0,0.22)" strokeWidth="1.5"/>
    {/* Serif "O" initial — Georgia, vertically and horizontally centred */}
    <text x="50" y="50" textAnchor="middle" dominantBaseline="central"
      fontFamily="Georgia, 'Times New Roman', serif"
      fontSize="66" fontWeight="700" fill="#FFFFFF">O</text>
  </svg>
);

// Wordmark — matches original pill wordmark but as true vector
const OkombaWordmark = ({ height = 36 }) => {
  const pill = { height, width: height * 3.8, rx: height * 0.28 };
  const fs1  = height * 0.52;
  const fs2  = height * 0.26;
  return (
    <svg width={pill.width} height={pill.height} viewBox={`0 0 ${pill.width} ${pill.height}`}
      fill="none" xmlns="http://www.w3.org/2000/svg"
      role="img" aria-label="OKOMBA Analytics">
      <rect width={pill.width} height={pill.height} rx={pill.rx} fill="#0B0F1A"/>
      <rect width={pill.width} height={pill.height} rx={pill.rx}
        fill="none" stroke="rgba(240,165,0,0.2)" strokeWidth="1"/>
      <text x={pill.width * 0.08} y={pill.height * 0.64}
        fontFamily="'Syne',system-ui,sans-serif" fontWeight="800"
        fontSize={fs1} fill="#F2F4F8" letterSpacing="-0.03em">
        Okomba
      </text>
      <text x={pill.width * 0.088} y={pill.height * 0.9}
        fontFamily="'DM Mono',monospace" fontWeight="500"
        fontSize={fs2} fill="#F0A500" letterSpacing="0.12em">
        ANALYTICS
      </text>
    </svg>
  );
...
// Full horizontal lockup — new brand wordmark (Animating-my-logo.svg)
// viewBox is tightened to the pill content area (removes whitespace canvas)
const OkombaLogoFull = ({ height = 36 }) => {
  const vbW = 420, vbH = 128;
  ...
  return (
    <svg width={width} height={height}
      ...
      role="img" aria-label="OKOMBA Analytics">
      {/* Pill — dark background with subtle gold border */}
      <rect x="195" y="55" width="410" height="118" rx="22" fill="#0B0F1A"/>
      <rect x="195" y="55" width="410" height="118" rx="22" fill="none" stroke="rgba(240,165,0,0.18)" strokeWidth="1"/>
      {/* "Okomba" — Georgia serif, bold, white */}
      <text x="400" y="135" textAnchor="middle"
        fontFamily="Georgia, 'Times New Roman', Times, serif"
        fontSize="96" fontWeight="700" fill="#FFFFFF" letterSpacing="-1">Okomba</text>
      {/* "Analytics" — Georgia serif, italic, brand gold */}
      <text x="585" y="162" textAnchor="end"
        fontFamily="Georgia, 'Times New Roman', Times, serif"
        fontSize="22" fontStyle="italic" fill="#F0A500">Analytics</text>
    </svg>
  );
};
        ...
        <div className="glass" style={{ borderRadius:"var(--radius-lg)", padding:"36px" }}>
          ...
          <form onSubmit={handleLogin} noValidate>
            ...
            <div className="input-group">
              ...
              <div style={{ position:"relative" }}>
                <input id="a-pass" className="input-field" type={showPassword ? "text" : "password"} placeholder="••••••••••••"
                  value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password"
                  style={{ paddingRight:"48px" }}/>
                <button type="button" onClick={() => setShowPassword(s => !s)}
                  style={{ position:"absolute", right:"14px", top:"50%", transform:"translateY(-50%)", background:"none", border:"none", cursor:"pointer", color:"var(--text-3)", padding:"4px", display:"flex", alignItems:"center", transition:"color 0.2s" }}
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <aside className={`admin-sidebar${sidebarOpen?" open":""}`} aria-label="Admin navigation">
        <div style={{ marginBottom:"28px", paddingBottom:"20px", borderBottom:"1px solid var(--bg-border)" }}>
          <OkombaLogoFull height={24}/>
          <div style={{ fontSize:"11px", color:"var(--text-3)", marginTop:"6px" }}>Admin Portal</div>
        </div>
